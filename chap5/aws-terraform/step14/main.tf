...
resource "aws_key_pair" "imported_key" {
  ...
  public_key = var.ci_config.ssh_key
}

# EC2 인스턴스 리소스 정의
resource "aws_instance" "web" {
  for_each      = toset(var.static_ips) # static_ips 리스트를 set으로 변환하여 각 IP에 대해 인스턴스 생성
  ami           = data.aws_ami.ubuntu.id
  instance_type = var.instance_type
  subnet_id     = aws_subnet.main.id
  private_ip    = each.value # 각 인스턴스에 static_ips의 IP 할당
  ...
  user_data = <<-EOF
              #cloud-config
              hostname: ${var.instance_name}-${replace(each.value, ".", "-")} # Unique hostname
              users:
  ...
  tags = {
    Name = "${var.instance_name}-${replace(each.value, ".", "-")}"
  }
}
