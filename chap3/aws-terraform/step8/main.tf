...
resource "aws_subnet" "main" {
  ...
  availability_zone = "${var.aws_region}a" # 예시 AZ
...
resource "aws_instance" "web" {
  ...
  subnet_id     = aws_subnet.main.id
  private_ip    = var.static_private_ip # Proxmox의 static_ip에 해당
  ...
  # Cloud-Init 스크립트.
  # AWS는 DHCP를 사용하므로 Cloud-Init에서 네트워크를 정적으로 설정할 필요 없음.
  # 인스턴스에 할당된 private_ip를 Cloud-Init이 인식하고 사용함.
  user_data = <<-EOF
              #cloud-config
