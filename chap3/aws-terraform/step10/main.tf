...
resource "aws_instance" "web" {
  ...
  subnet_id     = aws_subnet.main.id # VPC 및 서브넷 사용
  
  # 정적 Private IP는 step8에서 다루었으므로 여기서는 DHCP 사용 (Proxmox ipconfig0 = "ip=dhcp" 에 해당)
  # AWS는 기본적으로 DHCP를 사용하므로 private_ip를 명시적으로 지정하지 않음.
