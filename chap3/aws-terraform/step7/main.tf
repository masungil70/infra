...
resource "aws_instance" "web" {
  ...
  # Proxmox의 'cicustom'과 'cloud_init.yml' 내용을 user_data에 통합
  user_data = <<-EOF
              #cloud-config
              ...
              # Original cloud_init.yml content (adapted for AWS)
              # Qemu-guest-agent is not needed for AWS EC2.
              # runcmd:
              #   - apt update
              #   - apt install -y qemu-guest-agent
              #   - systemctl start qemu-guest-agent
              #   - systemctl enable qemu-guest-agent
              # Disable further Cloud-Init runs, if desired.
              runcmd:
