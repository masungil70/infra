...
resource "aws_vpc" "main" {
  ...
  tags = {
    Name = "${var.aws_region}-vpc" # Generic name for VPC in module
  }
}
...
resource "aws_security_group" "ssh_access" {
  name_prefix = "${var.aws_region}-ssh-" # Generic name for SG in module
  vpc_id      = aws_vpc.main.id
...
resource "aws_key_pair" "imported_key" {
  key_name   = "shared-ec2-key" # Common key for all instances in this module
